...
console.log("data",name, description, price,availableQty,manufacturer,model,primaryImageUrl,secondaryImageUrl);
        vehicles.push({ name, description, price,availableQty,manufacturer,model,primaryImageUrl,secondaryImageUrl });
      });

      for (const { name, description, price,availableQty,manufacturer,model,primaryImageUrl,secondaryImageUrl} of vehicles) {
console.log(manufacturer);
        const response1 = await axios.get(primaryImageUrl, { responseType: "stream" });
        ...
        await minioClient.putObject(bucket, primaryImagePath, primaryImageStream, {
          "Content-Type": "jpg/jpeg/png" || "application/octet-stream", // Adjust as needed
        });
        ...
        // Download the image
        const response = await axios.get(secondaryImageUrl, { responseType: "stream" });
        const secondaryImageStream = response.data;

        const secondaryImagePath = `vehicles/${name}/${name}.jpg`; // Change as needed
        await minioClient.putObject(bucket, secondaryImagePath, secondaryImageStream, {
          "Content-Type": "jpg/jpeg/png" || "application/octet-stream", // Adjust as needed
        });
  ...
  modelId,
    ...
    createVehicle: async (
      ...
      const newImage = await saveImageToDB({
        name,
        description,
        price,
        primaryImage,
